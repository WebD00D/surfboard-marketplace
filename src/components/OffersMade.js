...
class OffersMade extends PureComponent {
  ...
  componentDidMount() {
    ...
    var messageRef = fire
      .database()
      .ref('/users/' + this.props.userId + '/offersMade');
  ...
  render() {
    ...
    _.forEach(
      offersMadeDataSource,
      function(v, k) {
        console.log('DETAILS ABOUT THE SPECIFIC OFFER', v);

        const amount = parseFloat(v.amountOffered) * 100;
        const paymentLink = `/pay/?sellerId=${v.sellerId}&boardId=${
          v.boardId
        }&boardName=${v.boardName}&amount=${amount}`;
          ...
          <div key={k} style={{ marginBottom: '28px' }}>
            ...
            <div className="offers__row">
              <div>
                ...
                <div>
                  <Moment format="MM/DD/YYYY @ hh:mm A" date={v.offerDate} />
                </div>
              ...
              <div>
                {!v.offerAccepted ? (
                  <div
                    className="fc-yellow t-sans fw-500"
                    style={{ fontStyle: 'italic', paddingRight: '30px' }}
                  >
                    Offer Pending
                  </div>
                ) : (
